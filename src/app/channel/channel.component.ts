...
export class ChannelComponent implements OnInit {
  ...
  addUser(id) {
    this.service.addUser(this.channel._id, id)
      .subscribe((data: any) => {
        console.log(data)
  ...
  removeUser(id) {
    this.service.removeUser(this.channel._id, id)
      .subscribe((data: any) => {
        console.log(data)
        const items = this.channel.users.filter(element => element._id === id)
        this.otherUsers.push(items.shift())
        this.channel.users = this.channel.users.filter(element => element._id !== id)
      })
  }
